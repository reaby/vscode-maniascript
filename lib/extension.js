...
exports.activate = function (context) {
  ...
  var subscriptions = [];
  let timeout = null;
  let confValue = vscode.workspace.getConfiguration('', context.uri).get('conf.maniascript.apiDocPath');
  docParser.parseFile(null); // load default config
  if (confValue != null) {
    if (fs.existsSync(confValue)) {          
        docParser.parseFile(confValue);
        console.log("using external file for API completions:"+ confValue);    
    }
  }
  ...
  const onActiveEditorChange = function (editor) {
    if (editor) {
      ...
      let confValue = vscode.workspace.getConfiguration('', editor.uri).get('conf.maniascript.apiDocPath');
      docParser.parseFile(null); // load default config
      if (confValue != null) {        
        if (fs.existsSync(confValue)) {
            docParser.parseFile(confValue);
