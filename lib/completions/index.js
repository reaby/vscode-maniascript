...
const { findTypesInContext, findInContext, getClasses, classMethods, getClassNames, classProps, classEnums, getClassPropTypes } = require('./classes.js');
const { allVariables, getVarType, getAllVariableItems } = require('./variables.js');
const { parseFunctions, getAllFunctions } = require('./functions.js');
const { allNamespacesIncluded, isNamespace, getNamespace, namespacesElems } = require('./namespaces.js');
const { checkStructs, getStructElems, isStruct, getAllStructs, getStructElemType } = require('./structs.js');

const { getPrimitives } = require('./primitives');
...
function find(searchFor, docText) {
  ...
  requireContext = docText.match(/(#RequireContext\s+\w+)|(\@context\s+\w+)/m);
  ...
  if (lastElem.indexOf('.') > -1) {
    ...
    // find first element inside [] brackets
    // if element has only [, we need to autocomplete.
    if (firstElem.indexOf("[") !== -1 && firstElem.indexOf("]") == -1) {
      firstElem = firstElem.slice(firstElem.indexOf("[") + 1, firstElem.length);
    }

    firstElem = firstElem.replace(/\[(.*?)\]/g, "");
    var resolvedVariable = getStructElemType(firstElem) || getVarType(firstElem) || findTypesInContext(firstElem, requireContext) || "";
  ...
  if ((searchFor[0] === "declare" || searchFor[0] === "#Struct" || searchFor[0] === "#Const")) {
    if (searchFor.length == 2) {
      return [...getPrimitives(), ...getClasses(), ...getAllStructs(), new Item("persistent", Kind.Keyword)];
    } else {
      return [];
    }
  }
  ...
  return [...findFromDelimeter('.', requireContext), ...getClasses(), ...getAllStructs(), ...namespaces, ...keywords, ...variables, ...getAllFunctions(), ...getPrimitives()];
}
